...
package com.jeeframework.jeetask.config.simple;
...
public final class JobConfiguration implements JobRootConfiguration {
    ...
    public String getJobName() {
        return getJobDefinition().getJobName();
    }
    ...
    public boolean isFailover() {
        return monitorExecution && failover;
    }

    /**
     * 创建Lite作业配置构建器.
     *
     * @param jobDefinition 作业配置
     * @return Lite作业配置构建器
     */
    public static Builder newBuilder(final JobDefinition jobDefinition) {
        return new Builder(jobDefinition);
    }
    ...
    public static class Builder {
         ...
         * 设置监控作业执行时状态.
         * <p>
         * <p> 每次作业执行时间和间隔时间均非常短的情况, 建议不监控作业运行时状态以提升效率, 因为是瞬时状态, 所以无必要监控. 请用户自行增加数据堆积监控. 并且不能保证数据重复选取, 应在作业中实现幂等性.
         * 也无法实现作业失效转移. 每次作业执行时间和间隔时间均较长短的情况, 建议监控作业运行时状态, 可保证数据不会重复选取. </p>
         *
...
//            return new JobCoreConfiguration(jobName, jobParameter, failover, misfire, description, jobProperties);
////             return new JobCoreConfiguration(jobName, cron, shardingTotalCount, shardingItemParameters,
...
//    }
        ...
        public final JobConfiguration build() {
            Preconditions.checkArgument(!Strings.isNullOrEmpty(jobDefinition.getJobName()), "jobName can not be empty" +
                    ".");
            Preconditions.checkArgument(!Strings.isNullOrEmpty(jobDefinition.getJobClass()), "jobClass can not be " +
                    "empty.");
            return new JobConfiguration(jobDefinition, monitorExecution, maxTimeDiffSeconds, monitorPort,
