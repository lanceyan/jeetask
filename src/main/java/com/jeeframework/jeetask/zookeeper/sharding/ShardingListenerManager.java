...
import com.jeeframework.jeetask.zookeeper.config.ConfigurationNode;
    ...
    class ShardingTotalCountChangedJobListener extends AbstractJobListener {
        ...
        protected void dataChanged(final String path, final TreeCacheEvent.Type eventType, final String data) {
            if (configNode.isConfigPath(path) && 0 != JobRegistry.getInstance().getCurrentShardingTotalCount(jobName)) {
                int newShardingTotalCount = 1;//LiteJobConfigurationGsonFactory.fromJson(data).getTypeConfig()
