...
import com.jeeframework.jeetask.config.JobRootConfiguration;
import com.jeeframework.jeetask.config.simple.BizJobDefinition;
import listener.JavaSimpleJob;
...
//        JobTypeConfiguration typeConfig = getJobTypeConfiguration(coreConfig, jobType, jobClass, streamingProcess,
//                scriptCommandLine);
        return getJobConfiguration(bizJobConfig, customizedValueMap);
    }



    protected abstract void addToCustomizedValueMap(final String jsonName, final JsonReader in, final Map<String,
            Object> customizedValueMap) throws IOException;

//    private JobCoreConfiguration getJobCoreConfiguration(final String jobName, final String cron, final int
//            shardingTotalCount,
//                                                         final String shardingItemParameters, final String
//                                                                 jobParameter, final boolean failover,
//                                                         final boolean misfire, final String description,
//                                                         final JobProperties jobProperties) {
////        JobConfiguration(jobDefinition, monitorExecution, maxTimeDiffSeconds, monitorPort,
////                jobShardingStrategyClass, reconcileIntervalMinutes, disabled, overwrite, jobParameter, failover,
////                misfire, description, jobProperties);
//
//
//        return JobCoreConfiguration.newBuilder(jobName).jobParameter(jobParameter).failover(failover).misfire
//                (misfire).description(description)
//                .jobProperties(JobProperties.JobPropertiesEnum.JOB_EXCEPTION_HANDLER.getKey(), jobProperties.get
//                        (JobProperties.JobPropertiesEnum.JOB_EXCEPTION_HANDLER))
//                .jobProperties(JobProperties.JobPropertiesEnum.EXECUTOR_SERVICE_HANDLER.getKey(), jobProperties.get
...
////                .shardingItemParameters(shardingItemParameters).jobParameter(jobParameter).failover(failover)
//// .misfire(misfire).description(description)
////                .jobProperties(JobProperties.JobPropertiesEnum.JOB_EXCEPTION_HANDLER.getKey(), jobProperties.get
//// (JobProperties.JobPropertiesEnum.JOB_EXCEPTION_HANDLER))
////                .jobProperties(JobProperties.JobPropertiesEnum.EXECUTOR_SERVICE_HANDLER.getKey(), jobProperties.get
//// (JobProperties.JobPropertiesEnum.EXECUTOR_SERVICE_HANDLER))
...
//            final JobCoreConfiguration coreConfig, final JobType jobType, final String jobClass, final boolean
//            streamingProcess, final String scriptCommandLine) {
...
//                return new BizJobDefinition(coreConfig, jobClass);
//
//            default:
...
//    }
    ...
    protected abstract T getJobConfiguration(final JobDefinition jobDefinition, final Map<String, Object>
            customizedValueMap);

    @Override
    public void write(final JsonWriter out, final T value) throws IOException {
        out.beginObject();
        out.name("jobName").value(value.getJobDefinition().getJobName());
        out.name("jobClass").value(value.getJobDefinition().getJobClass());
